...
def test_cropping():
    entire_screen_pil = Image.open(os.path.join(get_dir('tests'), 'screenshot1.png'))
...
def test_table_scraper():
    ...
    table_scraper.screenshot = Image.open(os.path.join(get_dir('tests'), 'screenshot1.png'))
    table_scraper.crop_from_top_left_corner()
    table_scraper.is_my_turn()
    table_scraper.lost_everything()
    table_scraper.get_my_cards()
    table_scraper.get_table_cards()
    table_scraper.get_dealer_position()
    ...
    table_scraper.all_in_call_button()
